...
class AdminController extends Controller
{
    ...
    public function deleteSeasonAction($id)
    {
        ...
        //On cherche si la saison existe
        if ($season = $repo->find($id)) {
            //On récupère l'id
            ...
            //On cherche les épisodes et on les supprime
            $episodes = $season->getEpisodes();
            foreach ($episodes as $episode) {
                $em->remove($episode);
            }
            $em->flush();
            //On retourne sur la page de la saison actualisée
            return $this->redirect($this->generateUrl('show', ['id' => $showId]));
        } else {
